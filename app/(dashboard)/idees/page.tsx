...
const STATUT_CONFIG: Record<IdeaStatut, { label: string; color: string; badge: string }> = {
  capture:           { label: 'Capturée',          color: 'bg-stone-100',    badge: 'bg-stone-100 text-stone-500' },
  a_challenger:      { label: 'À challenger',       color: 'bg-amber-50',     badge: 'bg-amber-50 text-amber-600' },
  en_evaluation:     { label: 'En évaluation',      color: 'bg-blue-50',      badge: 'bg-blue-50 text-blue-600' },
  liee_projet:       { label: 'Liée à un projet',   color: 'bg-violet-50',    badge: 'bg-violet-50 text-violet-600' },
  transformee_tache: { label: 'Tâche créée',        color: 'bg-green-50',     badge: 'bg-green-50 text-green-600' },
  transformee_projet:{ label: 'Projet créé',        color: 'bg-green-50',     badge: 'bg-green-50 text-green-600' },
  en_attente:        { label: 'En attente',         color: 'bg-stone-50',     badge: 'bg-stone-50 text-stone-400' },
  abandonnee:        { label: 'Abandonnée',         color: 'bg-stone-50',     badge: 'bg-stone-50 text-stone-400' },
}
...
const NEXT_ACTIONS: Partial<Record<IdeaStatut, { label: string; next: IdeaStatut }[]>> = {
  ...
  a_challenger:  [{ label: 'Évaluer',    next: 'en_evaluation' }, { label: 'Abandonner', next: 'abandonnee' }],
...
export default function IdeesPage() {
  ...
  async function load() {
    const [{ data: ideesData }, { data: projetsData }] = await Promise.all([
      ...
      supabase.from('projets').select('id, nom').eq('statut', 'actif'),
    ])
  ...
  async function create() {
    ...
    if (data) setIdees(prev => [data as Idee, ...prev])
    setNewTexte('')
    setShowForm(false)
    setSaving(false)
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-stone-800">Idées</h1>
          ...
          return (
            <div key={idee.id} className="bg-surface border border-border rounded-2xl overflow-hidden">
              ...
              >
                ...
                {isOpen ? <ChevronUp size={15} className="text-muted shrink-0" /> : <ChevronDown size={15} className="text-muted shrink-0" />}
              </button>
      ...
      {done.length > 0 && (
        <div>
          ...
          >
            ...
            <span className="font-semibold uppercase tracking-wider">Archivées</span>
